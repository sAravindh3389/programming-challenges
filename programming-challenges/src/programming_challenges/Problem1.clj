(ns programming-challenges.Problem1)
...
; input and then the maximum cycle length for integers between and including i and j.
; These three numbers should be separated by one space, with all three numbers on one line and with
...
(defn get-cycle-length [n result]
  (loop [value-after-applying (apply-rule n) collection result]
    (if-not (= 1 value-after-applying)
      (do
        (recur (apply-rule value-after-applying) (conj collection value-after-applying)))
      (conj collection 1)))
  )
(defn start []   (println ( map get-cycle-length (range 1 100) [])))
